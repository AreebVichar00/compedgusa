...
if ( ! function_exists( 'erica_get_theme_mod' ) ) {

	function erica_get_theme_mod( $key = '', $default = '' ) {
		$mods = get_theme_mods();
		...
		return apply_filters( 'erica/filter/erica_get_theme_mode', $value, $key, $default, $mods );
	}

}

if ( ! function_exists( 'erica_get_option' ) ) {

	function erica_get_option( $key = '', $default = '' ) {
		$theme_options = get_option( 'erica_options', array() );
		...
		return apply_filters( 'erica/filter/get_option', $value, $key, $default, $theme_options );
	}
...
if ( ! function_exists( 'erica_render_variable' ) ) {
	function erica_render_variable( $variable ) {
		return $variable;
	}
}
 ...
 * @return string
 */
if ( !function_exists('erica_render_style_attribute') ){
    function erica_render_style_attribute( $inline_css ) {
        if(!empty($inline_css)){
...
if ( ! function_exists( 'erica_transfer_text_to_format' ) ) {
	function erica_transfer_text_to_format( $content, $autop = false ) {
		if ( $autop ) {
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'erica_comment' ) ) {

	function erica_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) :
			...
			case 'trackback' :
                ...
                <div id="comment-<?php comment_ID(); ?>" class="comment_container">
                    <p><?php esc_html_e( 'Pingback:', 'erica' ); ?>
                        <span><span><?php comment_author_link(); ?></span></span> <?php edit_comment_link( esc_html__( '(Edit)', 'erica' ), '<span class="edit-link">', '</span>' ); ?>
                    </p>
                </div>
            ...
            <div id="comment-<?php comment_ID(); ?>" class="comment-div">
                ...
                <div <?php comment_class( 'comment_container' ); ?>>
					...
					<?php echo get_avatar( $comment, apply_filters( 'erica_comment_avatar_size', 150 ) ); ?>

                    <div class="comment-text">

                        <div class="meta">
                            <div class="woocommerce-review__author"><?php printf( esc_html__( '%s ', 'erica' ), sprintf( '%s', get_comment_author_link() ) ); ?></div>
                            <span class="woocommerce-review__dash">–</span>
                            <span class="woocommerce-review__published-date"><?php comment_date( 'j M Y' ); ?></span>
                        </div>

                        <?php if ( '0' == $comment->comment_approved ) : ?>
                            <p class="description comment-entry comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'erica' ); ?></p>
                        <?php endif; ?>
                        ...
                        <span class="comment-footer">
                            ...
                            <?php edit_comment_link( __( 'Edit', 'erica' ) ); ?>
                        </span>
                    </div>
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'erica_modify_comment_form_fields' ) ) {

	function erica_modify_comment_form_fields( $fields ) {

		$commenter = wp_get_current_commenter();
		$req       = get_option( 'require_name_email' );

		$fields['author'] = '<div class="comment-form-author"><input type="text" name="author" id="author" value="' . esc_attr( $commenter['comment_author'] ) . '" placeholder="' . esc_attr__( 'Name (required)', 'erica' ) . '" size="22" tabindex="101"' . ( $req ? ' aria-required="true"' : '' ) . ' class="input-name" /></div>';

		$fields['email'] = '<div class="comment-form-email"><input type="text" name="email" id="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" placeholder="' . esc_attr__( 'Email', 'erica' ) . '" size="22" tabindex="102"' . ( $req ? ' aria-required="true"' : '' ) . ' class="input-email" /></div>';

		$fields['url'] = '<div class="comment-form-url"><input type="text" name="url" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" placeholder="' . esc_attr__( 'Website', 'erica' ) . '" size="22" tabindex="103" class="input-website" /></div>';

		return $fields;

	}

	add_filter( 'comment_form_default_fields', 'erica_modify_comment_form_fields' );
 ...
 * String to boolean
 */
if ( ! function_exists( 'erica_string_to_bool' ) ) {
	function erica_string_to_bool( $string ) {
 ...
 */
if ( ! function_exists( 'erica_get_post_thumbnail_format' ) ) {
	function erica_get_post_thumbnail_format( $thumbnail_size = 'full', $post_link = true ) {
		...
		switch ( $post_format ) {
			...
			case 'quote':
				...
				if ( ! empty( $_quote_text ) ) {
					...
					if(has_post_thumbnail()){
						$format_output = sprintf('<div class="postformat-content postformat-content--standard postformat-content--quote" %3$s><div class="postformat-content--inner">%4$s</div><div class="postformat-content--quote-inner"><div class="postformat-content-text">%1$s</div>%2$s</div></div>', esc_html( $_quote_text ), $_quote_cite, erica_render_style_attribute($_quote_style), get_the_post_thumbnail( $post_id, $thumbnail_size ) );
                    }
					else{
						$format_output = sprintf( '<div class="postformat-content postformat-content--quote" %3$s><div class="postformat-content--quote-inner"><div class="postformat-content-text">%1$s</div>%2$s</div></div>', esc_html( $_quote_text ), $_quote_cite, erica_render_style_attribute($_quote_style) );
                    }
...
if ( ! function_exists( 'erica_entry_meta_item_category_list' ) ) {
	function erica_entry_meta_item_category_list( $before = '', $after = '', $separator = ', ', $parents = '', $post_id = false ) {
		add_filter( 'get_the_terms', 'erica_exclude_demo_term_in_category' );
		$categories_list = get_the_category_list( '{{_}}', $parents, $post_id );
		erica_deactive_filter( 'get_the_terms', 'erica_exclude_demo_term_in_category' );
		if ( $categories_list ) {
			printf(
				'%3$s<span class="screen-reader-text">%1$s </span><span>%2$s</span>%4$s',
				esc_html_x( 'Posted in', 'front-view', 'erica' ),
...
if ( ! function_exists( 'erica_exclude_demo_term_in_category' ) ) {
	function erica_exclude_demo_term_in_category( $term ) {
		return apply_filters( 'erica/post_category_excluded', $term );
	}
}

if ( ! function_exists( 'erica_deactive_filter' ) ) {
	function erica_deactive_filter( $tag, $function_to_remove, $priority = 10 ) {
		return call_user_func( 'remove_filter', $tag, $function_to_remove, $priority );
	}
}

if ( ! function_exists( 'erica_wpml_object_id' ) ) {
	function erica_wpml_object_id( $element_id, $element_type = 'post', $return_original_if_missing = false, $ulanguage_code = null ) {
...
if ( ! function_exists( 'erica_is_blog' ) ) {
	function erica_is_blog() {
		return ( is_home() || is_tag() || is_category() || is_date() || is_year() || is_month() || is_author() ) ? true : false;
	}
}

if ( ! function_exists( 'erica_get_wishlist_url' ) ) {
	function erica_get_wishlist_url() {
		$wishlist_page_id = erica_get_theme_mod( 'wishlist_page', 0 );

		return ( ! empty( $wishlist_page_id ) ? get_the_permalink( $wishlist_page_id ) : esc_url( home_url( '/wishlist/' ) ) );
	}
}

if ( ! function_exists( 'erica_get_compare_url' ) ) {
	function erica_get_compare_url() {
		$compare_page_id = erica_get_theme_mod( 'compare_page', 0 );

		return ( ! empty( $compare_page_id ) ? get_the_permalink( $compare_page_id ) : esc_url( home_url( '/compare/' ) ) );
	}
}

if( !function_exists('erica_minimizeCSS') ) {
    function erica_minimizeCSS( $css ){

...
if (!class_exists('Erica_Ajax_Manager')) {
    ...
    class Erica_Ajax_Manager
    {
	    ...
	    public function handle_ajax_request() {
		    if ( ! $this->verify_request_nonce() ) {
			    ...
			    $this->add_response_data( false, esc_html__( 'Token Expired.', 'erica' ) )
			         ->send_error( 401 );
		    }

		    if(empty($_REQUEST['actions'])){
			    $this->add_response_data( false, esc_html__( 'Action not found.', 'erica' ) )
			         ->send_error( 401 );
		    }
		    ...
		    foreach ( $this->requests as $id => $action_data ) {
			    ...
			    if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				    $this->add_response_data( false, esc_html__( 'Action not found.', 'erica' ), 400 );
	    ...
	    private function send_success() {
		    ...
		    if ( function_exists( 'gzencode' ) ) {
			    ...
			    echo erica_render_variable($response); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		    } else {
			    echo erica_render_variable($json); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		    }
	     ...
	     *
	     * @return Erica_Ajax_Manager An instance of ajax manager.
	     */

<?php
/**
 * Erica Breadcrumb Trail class
 *
 * @package Erica WordPress theme
 * @since 1.0.0
 */

if ( !function_exists('erica_breadcrumb_trail') ) {
    function erica_breadcrumb_trail( $args = array() ) {

        // Return if breadcrumbs are disabled
        if ( ! apply_filters( 'erica/filter/display_breadcrumbs', true ) || is_front_page() ) {
            return;
        }
        ...
        if ( !is_object( $breadcrumb ) )
            $breadcrumb = new Erica_Breadcrumb_Trail( $args );
...
if( !function_exists('erica_sp_breadcrumbs_before') ) {
    function erica_sp_breadcrumbs_before() {
        $classes = 'site-breadcrumbs';
        echo '<div class="'. esc_attr($classes) .'">';
    }
}
add_action( 'seopress_breadcrumbs_before_html', 'erica_sp_breadcrumbs_before' );

if(!function_exists('erica_sp_breadcrumbs_after')){
    function erica_sp_breadcrumbs_after() {
        echo '</div>';
    }
}
add_action( 'seopress_breadcrumbs_after_html', 'erica_sp_breadcrumbs_after' );
...
if( !class_exists('Erica_Breadcrumb_Trail') ) {

    class Erica_Breadcrumb_Trail {
        ...
        /**
         * Sets up the breadcrumb trail properties.  Calls the `Erica_Breadcrumb_Trail::add_items()` method
         ...
         *     @type array     $labels         Text labels. @see Erica_Breadcrumb_Trail::set_labels()
         *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Erica_Breadcrumb_Trail::set_post_taxonomy()
        ...
        public function trail() {
            ...
            echo erica_render_variable( $this->get_trail() );
        }
        ...
        protected function set_labels() {
            ...
            $defaults = array(
                'aria_label'          	=> esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'erica' ),
                'home'           		=> esc_html__( 'Home', 'erica' ),
                'error_404'           	=> esc_html__( '404 Not Found', 'erica' ),
                'archives'            	=> esc_html__( 'Archives',                              'erica' ),
                // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
                'search'           		=> esc_html__( 'Search results for', 'erica' ),
                // Translators: %s is the page number.
                'paged'               	=> esc_html__( 'Page %s',                               'erica' ),
                // Translators: Minute archive title. %s is the minute time format.
                'archive_minute'      	=> esc_html__( 'Minute %s',                             'erica' ),
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week'        	=> esc_html__( 'Week %s',                               'erica' ),
        ...
        protected function add_network_home_link() {
            ...
            if ( is_multisite() && ! is_main_site() && true === $this->args['network'] )
                $this->items[] = sprintf( '<a href="%s" rel="home" aria-label="' .$this->labels['home']. '" %s>%s</a>', esc_url( network_home_url() ), $icon, erica_get_schema_markup('item'), $text );
        }
        ...
        protected function add_minute_hour_archive_items() {
            ...
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'erica' ) ) );
        }
        ...
        protected function add_minute_archive_items() {
            ...
            // Add the minute item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'erica' ) ) );
        }
        ...
        protected function add_hour_archive_items() {
            ...
            // Add the hour item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'erica' ) ) );
        }
        ...
        protected function add_day_archive_items() {
            ...
            $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'erica' ) ) );
            $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'erica' ) ) );
            $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'erica' ) ) );
        ...
        protected function add_week_archive_items() {
            ...
            $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'erica' ) ) );
            $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'erica' ) ) );
        ...
        protected function get_post_types_by_slug( $slug ) {
            ...
            foreach ( $post_types as $type ) {
                ...
                if ( $slug === $type->has_archive || ( true === $type->has_archive && !empty($type->rewrite['slug'] ) && $slug === $type->rewrite['slug'] ) ){
                    $return[] = $type;
                }
        ...
        protected function map_rewrite_tags( $post_id, $path ) {
            ...
            if ( is_array( $matches ) ) {
                ...
                foreach ( $matches as $match ) {
                    ...
                    if ( '%year%' == $tag )
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'erica' ) ) ) );

                    // If using the %monthnum% tag, add a link to the monthly archive.
                    elseif ( '%monthnum%' == $tag )
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'erica' ) ) ) );

                    // If using the %day% tag, add a link to the daily archive.
                    elseif ( '%day%' == $tag )
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ),  sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'erica' ) ) ) );

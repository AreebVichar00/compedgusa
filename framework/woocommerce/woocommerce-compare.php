...
if(!class_exists('Erica_WooCommerce_Compare')){
    class Erica_WooCommerce_Compare {
	    ...
	    /**
	     * @param Erica_Ajax_Manager $ajax_manager
	     *
	     * @return void
	     */
	    ...
	    public function new_ajax_process( $request = [] ){
		    ...
		    if(!empty($request['type']) && $request['type'] == 'load'){
			    ...
			    $return_data['message'] = esc_html__('Invalid Product ID', 'erica');
		    ...
		    $return_data['compare_url'] = erica_get_compare_url();
        ...
        public function remove_item_from_url(){
            if(isset($_GET['la_helpers_compare_remove']) && ( $product_id = absint($_GET['la_helpers_compare_remove'])) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been removed from list', 'erica'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        private function add( $post_id = 0 ){
            ...
            if(in_array($post_id, $lists)){
                $response['message'] = esc_html__('Product already in list', 'erica');
                $response['has_error'] = true;
            }
            else{
                $response['message'] = sprintf( esc_html__('"%1$s" has been added', 'erica'), get_the_title($post_id) );
        ...
        private function remove( $post_id = 0 ){
            ...
            if(($key = array_search($post_id, $lists)) !== false) {
                $response['message'] = esc_html__('Product has been removed from list', 'erica');
                $response['has_error'] = false;
                unset($lists[$key]);
                $lists = array_values($lists);
                $this->set_lists_for_user($lists);
                ...
                $response['message'] = esc_html__('Product does not exist in list', 'erica');
                $response['has_error'] = true;
            }
            return $response;
        }
        ...
        public static function get_default_attributes(){
            return array(
                'image'         => esc_html__( 'Image', 'erica' ),
                'title'         => esc_html__( 'Title', 'erica' ),
                'add-to-cart'   => esc_html__( 'Add to cart', 'erica' ),
                'price'         => esc_html__( 'Price', 'erica' ),
                'sku'           => esc_html__( 'Sku', 'erica' ),
                'description'   => esc_html__( 'Description', 'erica' ),
                'stock'         => esc_html__( 'Availability', 'erica' ),
                'weight'        => esc_html__( 'Weight', 'erica' ),
                'dimensions'    => esc_html__( 'Dimensions', 'erica' )
            );
        }

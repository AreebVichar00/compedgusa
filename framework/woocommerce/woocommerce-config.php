...
if(!class_exists('Erica_WooCommerce_Config')){

    class Erica_WooCommerce_Config {
        ...
        public function __construct() {
            ...
            add_filter('erica/filter/sidebar_primary_name', array( $this, 'set_sidebar_for_shop'), 20 );
             ...
             * Catalog Mode
             */
            if( erica_string_to_bool( erica_get_theme_mod('catalog_mode') ) ){
                // In Loop
                add_filter( 'woocommerce_loop_add_to_cart_link', '__return_empty_string', 10 );
                // In Page
                add_action( 'wp', array( $this, 'set_page_when_active_catalog_mode' ) );

                if( erica_string_to_bool( erica_get_theme_mod('catalog_mode_price') ) ){
        ...
        public function register_sidebars(){
            ...
            register_sidebar( array(
                'name'			=> esc_html__( 'Sidebar - Shop', 'erica' ),
                'id'			=> 'sidebar-shop',
                'description'	=> esc_html__( 'Widgets in this area will be displayed in the shop page.', 'erica' ),
            ...
            register_sidebar( array(
                'name'			=> esc_html_x( 'Sidebar - Filter', 'admin-view',  'erica' ),
        ...
        public function theme_js(){
            ...
            $ext = apply_filters('erica/use_minify_js_file', false) || ( defined('WP_DEBUG') && WP_DEBUG ) ? '' : '.min';

            wp_register_script('erica-woocommerce', get_theme_file_uri( '/assets/js/lib/woocommerce'. $ext .'.js' ), array('jquery'), $theme_version, true);
        ...
        public function add_compare_btn(){
            ...
            if( erica_string_to_bool( erica_get_theme_mod('woocommerce_show_compare_btn') ) ) {
                if ( !empty($yith_woocompare->obj) ) {
                    ...
                    printf(
                        ...
                        esc_attr($css_class),
                        esc_url($url),
                        esc_attr_x('Add to compare','front-view', 'erica'),
                    ...
                    $text = esc_html_x('Add to compare','front-view', 'erica');
                    printf(
        ...
        public function add_wishlist_btn(){
	        ...
	        if( erica_string_to_bool( erica_get_theme_mod('woocommerce_show_wishlist_btn') ) ) {
                global $product;
                if (function_exists('YITH_WCWL')) {
                    ...
                    if (YITH_WCWL()->is_product_in_wishlist($product->get_id(), $default_wishlist)) {
                        $text = esc_html_x('View Wishlist', 'front-view', 'erica');
                        $class = 'add_wishlist la-yith-wishlist button added';
                        $url = YITH_WCWL()->get_wishlist_url('');
                    }
                    else {
                        $text = esc_html_x('Add to Wishlist', 'front-view', 'erica');
        ...
        public function change_per_page_default($cols){
            $per_page_array = erica_woo_get_product_per_page_array();
            $per_page = erica_woo_get_product_per_page();
        ...
        public function set_cookie_default(){
            if (isset($_GET['per_page'])) {
                add_filter('erica/filter/get_product_per_page', array( $this, 'get_parameter_per_page'));
            }
        }

        public function get_parameter_per_page($per_page) {
            if (isset($_GET['per_page']) && ($_per_page = $_GET['per_page'])) {
                $param_allow = erica_woo_get_product_per_page_array();
        ...
        public function add_custom_tabs( $tabs ){
            ...
            if(erica_string_to_bool(erica_get_option('woo_enable_custom_tab'))){
                $custom_tabs = erica_get_option('woo_custom_tabs');
        ...
        public function callback_custom_tab_content( $tab_key, $tab_instance ){
            if(!empty($tab_instance['custom_content'])){
                echo wp_kses_post( erica_transfer_text_to_format($tab_instance['custom_content'], true) );
            }
        }
        ...
        public function add_toolbar_close(){
            if(wc_get_loop_prop('lakit_loop_allow_extra_filters')){
                $view_mode = apply_filters('erica/filter/catalog_view_mode', 'grid');
                $woocommerce_toggle_grid_list = true;
                $active_shop_filter = true;
                $per_page_array = erica_woo_get_product_per_page_array();
                $per_row_array = erica_woo_get_product_per_row_array();
                $per_page =  erica_woo_get_product_per_page();
                ...
                if(!empty($per_row_array)){
                    ...
                    echo sprintf('<button><span data-text="%1$s">%1$s</span></button>', esc_html__('View', 'erica'));
                    echo '<ul>';
                    foreach ($per_row_array as $val){
                        echo sprintf('<li><a data-col="%3$s" href="javascript:;">%1$s %2$s</a></li>', esc_html__('View', 'erica'), ($val < 10 ? '0' . $val : $val), esc_html($val));
                    }
                    echo '</ul>';
                    echo '</div>';
                }
                if(!empty($per_page_array)){
                    ...
                    echo sprintf('<button><span data-text="%1$s">%1$s %2$s</span></button>', esc_html__('Show', 'erica'), esc_html($per_page));
                    echo '<ul>';
                    foreach ($per_page_array as $val){
                        echo sprintf('<li %4$s><a href="%3$s">%1$s %2$s</a></li>',
                            esc_html__('Show', 'erica'),
                            esc_html($val),
                ...
                if($active_shop_filter && is_active_sidebar('sidebar-shop-filter')){
                    echo sprintf('<div class="lasf-custom-dropdown wc-custom-filters"><button class="btn-advanced-shop-filter"><span>%1$s</span></button></div>', esc_html__('Filters', 'erica'));
                }
                if( erica_string_to_bool($woocommerce_toggle_grid_list) ):
                    ?>
                    <div class="wc-view-toggle">
                        <button data-view_mode="list"<?php if ($view_mode == 'list') echo ' class="active"';?>><i title="<?php echo esc_attr__('List view', 'erica') ?>" class="lastudioicon-list-bullet-2"></i></button>
                        <button data-view_mode="grid"<?php if ($view_mode == 'grid')  echo ' class="active"'; ?>><i title="<?php echo esc_attr__('Grid view', 'erica') ?>" class="lastudioicon-microsoft"></i></button>
                    </div>
                <?php
                endif;
                ...
                if (erica_string_to_bool($active_shop_filter) && is_active_sidebar('sidebar-shop-filter')):
                ?>
                        ...
                        <div class="sidebar-inner">
                            ...
                            <?php if( is_filtered() || (!is_filtered() && is_product_taxonomy() && isset($_GET['la_doing_ajax'])) ) : ?>
                                <div class="la-advanced-product-filters-result">
                                    <?php
                                    $base_filter = erica_get_base_shop_url();
                                    if(isset($_GET['la_preset'])){
                                        $base_filter = add_query_arg('la_preset', $_GET['la_preset'], $base_filter);
                                    }
                                    ?>
                                    <a class="reset-all-shop-filter" href="<?php echo esc_url($base_filter) ?>"><i class="lastudioicon-e-remove"></i><span><?php echo esc_html__('Clear All Filter', 'erica'); ?></span></a>
                                </div>
                            <?php endif; ?>
                        </div>
        ...
        public function set_posts_per_page_wc_builder( $query ){
            ...
            if ( ( ! is_admin() ) && ( $query === $wp_the_query ) && is_product_taxonomy()) {
                $query->set( 'posts_per_page',  $this->change_per_page_default(erica_woo_get_product_per_page()) );
            }

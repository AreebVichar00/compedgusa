...
if(!class_exists('Erica_WooCommerce_Wishlist')){
    class Erica_WooCommerce_Wishlist {
		...
		public function new_ajax_process( $request = [] ){
			...
			$return_data['wishlist_url'] = erica_get_wishlist_url();
        ...
        public function remove_item_from_url(){
            if(isset($_GET['la_helpers_wishlist_remove']) && ( $product_id = absint($_GET['la_helpers_wishlist_remove'])) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been removed from wishlist', 'erica'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        public function remove_from_wishlist_after_add_to_cart() {
            if( isset( $_REQUEST['la_remove_from_wishlist_after_add_to_cart'] ) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    $this->set_lists_for_cookie($product_exists);
                    self::$data = $product_exists;
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been added to your cart.', 'erica'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        private function add( $post_id = 0 ){
            ...
            if(in_array($post_id, $lists)){
                $response['message'] = esc_html__('Product already in wishlist', 'erica');
                $response['has_error'] = true;
            }
            else{
                $response['message'] = esc_html__('Added to the wishlist!', 'erica');
        ...
        private function remove( $post_id = 0 ){
            ...
            if(($key = array_search($post_id, $lists)) !== false) {
                $response['message'] = esc_html__('Product has been removed from wishlist', 'erica');
                $response['has_error'] = false;
                unset($lists[$key]);
                $lists = array_values($lists);
                $this->set_lists_for_user($lists);
                ...
                $response['message'] = esc_html__('Product does not exist in wishlist', 'erica');
                $response['has_error'] = true;
            }
            return $response;
        }
...
new Erica_WooCommerce_Wishlist();
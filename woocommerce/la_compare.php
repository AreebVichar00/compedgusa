...
<div id="la_compare_table_wrapper">
    ...
    $lists = Erica_WooCommerce_Compare::get_data();

    $default_attr = Erica_WooCommerce_Compare::get_default_attributes();
    $tax_attr = Erica_WooCommerce_Compare::get_taxonomies();
    $all_attr = array_merge($default_attr, $tax_attr);

    $attribute_allow = [];
    $s = erica_get_theme_mod('compare_attribute');
    $settings = [];
    if(!empty($s)){
        $settings = explode(',', $s);
    ...
    add_filter('erica/filter/enable_image_lazyload', '__return_false');
        ...
        foreach ($lists as $product_id) {
            ...
            $term['id'] = $product_id;
            $term['sku'] = ($_product->get_sku() ? $_product->get_sku() : esc_html__('N/A', 'erica'));
        ...
        <table class="la-compare-table la-compare-table-items has-<?php echo esc_attr($total_product); ?> show-vtb">
            ...
            foreach( $attribute_allow as $lk => $lb ){
                ...
                if(in_array($lk, array('image','title','add-to-cart'))){
                    $label_list['info'] = esc_html_x('Product Info', 'front-view', 'erica');
                }
                else{
                ...
                foreach ($terms as $item) {
                    ...
                    elseif ($k == 'stock') {
                        ...
                        if ($item['availability'] && !empty($item['availability']['availability'])) {
                            ...
                            if ($item['stock']) {
                                $text_stock = esc_html__('In stock', 'erica');
                                $class_stock = 'in-stock';
                            } else {
                                $text_stock = esc_html__('Out of stock', 'erica');

...
$lists = Erica_WooCommerce_Wishlist::get_data();
$total = count($lists);

if($noLimit){
	$wishlist_page_id = erica_get_theme_mod( 'wishlist_page', 0 );
    $wishlist_title = !empty($wishlist_page_id) ? get_the_title($wishlist_page_id) : esc_html__('Wishlist', 'erica');
...
if($total > 1){
    ...
    if( $pages > 1 ){
        $page_links = paginate_links( array(
            'base' => esc_url( add_query_arg( array( 'paged' => '%#%' ), erica_get_wishlist_url() ) ),
            'format' => '?paged=%#%',
            'current' => $current_page,
            'total' => $pages,
            'show_all' => true,
            'prev_text'    => esc_html_x('Prev', 'front-view', 'erica'),
            'next_text'    => esc_html_x('Next', 'front-view', 'erica'),
            'type'         => 'list'
        ) );
...
<table class="la_wishlist_table shop_table shop_table_responsive woocommerce-cart-form__contents" cellspacing="0">
    <thead>
        <tr>
            ...
            <th class="product-name"><?php esc_html_e( 'Product', 'erica' ); ?></th>
            <th class="product-price"><?php esc_html_e( 'Price', 'erica' ); ?></th>
            <th class="product-stock"><?php esc_html_e( 'Stock status', 'erica' ); ?></th>
    ...
    if($total > 0){
        ...
        foreach($newlist as $product_id){
            ...
            $product_id = erica_wpml_object_id ( $product_id, 'product', true );
                ...
                <tr class="woocommerce-cart-form__cart-item cart_item">
                    <td class="product-remove">
                        ...
                        echo sprintf(
                            ...
                            esc_attr__( 'Remove this item', 'erica' ),
                            esc_attr( $product_id ),
                    ...
                    ?></td>
                    <td class="product-name" data-title="<?php esc_attr_e( 'Product', 'erica' ); ?>"><?php
                    ...
                    ?></td>
                    <td class="product-price" data-title="<?php esc_attr_e( 'Price', 'erica' ); ?>"><?php
                        echo sprintf('<div class="wl-item--price">%1$s</div>', $_product->get_price_html());
                    ?></td>
                    <td class="product-stock" data-title="<?php esc_attr_e( 'Stock status', 'erica' ); ?>"><?php
                        echo sprintf('<div class="wl-item--stock">%1$s</div>', $stock_html);
                    ?></td>
        ...
        ?>
        <tr class="not-found-product text-center"><td colspan="4"><?php esc_html_e('No products were added to the wishlist', 'erica') ?></td></tr>
        <?php
    }
...
?>
    <div class="la_wishlist-bottom">
        <div class="la_wishlist-bottom_actions">
            <a class="lawl--page" href="<?php echo esc_url( erica_get_wishlist_url() ); ?>"><?php echo esc_html__('Open wishlist page', 'erica'); ?></a>
            <span class="lawl--continue"><?php echo esc_html__('Continue shopping', 'erica'); ?></span>
        </div>

(function ($) {
    ...
    LaStudioWooCommerce.prototype.General = function (){
        ...
        $document
            .on('click','.wc-view-item a',function(){
                ...
                if(!_this.hasClass('active')){
                    ...
                    Cookies.set('erica_wc_product_per_row', _col, { expires: 2 });
                }
            })
            .on('click','.wc-view-toggle button',function(){
                ...
                if(!_this.hasClass('active')){
                    ...
                    Cookies.set('erica_wc_catalog_view_mode', _mode, { expires: 2 });
                }
            })
        /**
         * Ajax add-to-cart
        ...
        $(document).on('submit', '.lakit-popup--content form.woocommerce-shipping-calculator', function (e){
            ...
            var calc_ajax = AjaxManager('erica_calculate_shipping', {
                beforeSendCallback: function (){
                    if($('.woocommerce-notices-wrapper', $popup).length == 0){
                        $popup.find('.lakit-popup--title').after('<div class="woocommerce-notices-wrapper"></div>');
                    }
            ...
            calc_ajax.data = $form.serialize() + '&action=erica_calculate_shipping';
            calc_ajax.send();
        });
        ...
        $(document).on('change', '.cart-totals-table select.shipping_method, .cart-totals-table :input[name^=shipping_method]', function (e){
            ...
            var calc_ajax = AjaxManager('erica_update_shipping', {
                successCallback: function (response, textStatus, jqXHR){
                    if ( response.data && response.data.fragments ) {
                        $.each( response.data.fragments, function( key, value ) {
                            $( key ).replaceWith( value );
    ...
    LaStudioWooCommerce.prototype.AjaxFilter = function (){
        ...
        function ajaxFilterSuccessCallback( response, url, element){
            ...
            for ( let i = 0; i < elm_to_replace.length; i++){
            ...
            try{
                let e_css = $.parseHTML(response).filter( s => s.id === 'elementor-frontend-inline-css' )[0];
                if(e_css){
                    $('#elementor-frontend-inline-css').replaceWith( $(e_css) );
                }
            }catch (e) { }

            // cache for no products found
            if( $('.woocommerce.lakit_wc_widget_current_query > .woocommerce-no-products-found').length || $(response).find('.woocommerce.lakit_wc_widget_current_query > .woocommerce-no-products-found').length ){
            ...
            try {
                var _view_mode = Cookies.get('erica_wc_catalog_view_mode');
                $('.wc-toolbar .wc-view-toggle button[data-view_mode="'+_view_mode+'"]').trigger('click');

                var _per_row = Cookies.get('erica_wc_product_per_row');
        ...
        $document
            ...
            .on('click', '.elementor-lakit-woofilters .lakit-woofilters-ul a', function(e){
                ...
                if( $li_parent.hasClass('active') && (_type === 'product_cat_kitfilter' || _type === 'product_tag_kitfilter') ){
                    ...
                    _preset_from_e = LaStudio.global.getUrlParameter('theme_template_id', _shop_url);
                    if(!_preset_from_e && _preset_from_w){
                        _shop_url = LaStudio.global.addQueryArg(_shop_url, 'theme_template_id', _preset_from_w);
                    }

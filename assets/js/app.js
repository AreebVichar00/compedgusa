(function ($) {
    ...
    LaStudio.global.isPageSpeed = function () {
        return typeof navigator !== "undefined" && (/(lighthouse|gtmetrix)/i.test(navigator.userAgent.toLocaleLowerCase()) || /moericaa\/5\.0 \(x11; linux x86_64\)/i.test(navigator.userAgent.toLocaleLowerCase()));
    };
...
(function ($) {
    ...
    LaStudio.core.Blog = function ($sidebar_inner) {
        ...
        $('li.current-cat, li.current-cat-parent, li.current-cat-ancestor', $sidebar_inner).each(function(){
            $(this).addClass('open');
    ...
    LaStudio.core.CustomFunction = function () {
        ...
        try {
            if (Cookies.get('erica_wishlist') !== 'undefined') {
                const _cW = JSON.parse(Cookies.get('erica_wishlist'));
                wishlist_compare_count.wishlist = _cW[0].posts.length
            }
            if (Cookies.get('erica_compare') !== 'undefined') {
                const _cC = JSON.parse(Cookies.get('erica_compare'));
        ...
        return {
            init: function () {
                if (waitfortrigger != 1) {
                    ...
                    try {
                        if (Cookies.get('erica_dont_display_popup') == 'yes') {
                            return;
                        }
                    } catch (ex) {
                        ...
                        setTimeout(function () {
                            LaStudio.core.OpenNewsletterPopup($popup, function () {
                                if ($('.cbo-dont-show-popup', $popup).length && $('.cbo-dont-show-popup', $popup).is(':checked')) {
                                    try {
                                        Cookies.set('erica_dont_display_popup', 'yes', {
                                            expires: backtime,
                                            path: '/'
                                        });
                                    } catch (ex) {
